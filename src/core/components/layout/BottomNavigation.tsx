...
export function BottomNavigation() {
  ...
  return (
    <nav className="sticky bottom-0 z-30 flex items-center h-14 bg-surface border-t border-border-light/60 lg:hidden">
      {navItems.map((item) => {
        const isActive =
        ...
        return (
          <button
            ...
            className={cn(
              'flex flex-1 flex-col items-center justify-center gap-0.5 py-1 transition-colors cursor-pointer relative',
          ...
          >
            ...
            {isActive && (
              <span className="absolute top-0 left-1/2 -translate-x-1/2 w-12 h-0.5 bg-accent rounded-full" />
            )}
          </button>
        )
      })}
