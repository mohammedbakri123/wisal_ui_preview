...
export function MemberCard({ user, role = 'Member', onClick }: MemberCardProps) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={!onClick}
      className="flex w-full items-center gap-3 rounded-xl border border-border-light/50 bg-surface p-3 text-left transition-all duration-150 enabled:hover:bg-surface-hover disabled:cursor-default"
    >
