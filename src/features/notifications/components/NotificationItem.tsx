...
export function NotificationItem({ item, onOpen }: NotificationItemProps) {
  return (
    <button
      type="button"
      onClick={() => onOpen(item)}
      className={`relative flex w-full items-start gap-3 overflow-hidden rounded-2xl border bg-surface p-4 text-left transition-all hover:bg-surface-hover hover:shadow-sm ${
        item.isUnread ? 'border-accent/30 shadow-sm shadow-accent/5' : 'border-border-light/40'
      }`}
    >
      {item.isUnread && <div className="absolute right-4 top-4 h-2 w-2 rounded-full bg-accent" />}
