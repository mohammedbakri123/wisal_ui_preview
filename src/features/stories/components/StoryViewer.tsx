    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 animate-fade-in">
      ...
      >
        ...
        <div className="absolute top-4 left-0 right-0 z-20 flex items-center gap-3 px-3 pt-1">
          ...
          {isOwner && (
            <button onClick={() => setShowOwnerMenu((visible) => !visible)} className="p-2 rounded-full text-white/80 hover:bg-white/10 cursor-pointer" aria-label="خيارات القصة">
        ...
        {isOwner && showOwnerMenu && (
          <div className="absolute end-3 top-16 z-40 w-44 rounded-xl border border-white/15 bg-black/85 p-1 text-sm shadow-xl backdrop-blur-md">
            <button onClick={() => setShowViewers(true)} className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-start text-white/85 hover:bg-white/10">
              عرض المشاهدين <span className="text-xs text-white/45">{story.viewedBy.length}</span>
            </button>
            <button onClick={() => { onDelete?.(story.id); onClose() }} className="w-full rounded-lg px-3 py-2 text-start text-red-300 hover:bg-red-500/15">
              حذف القصة
            </button>
          </div>
        )}
        ...
        <div className="flex-1 flex items-center justify-center p-8 z-0">
          ...
          {story.type === 'image' && story.mediaUrl && (
            <img src={story.mediaUrl} alt="قصة" className="max-w-full max-h-full object-contain rounded-lg" />
          )}
          ...
          {story.type !== 'text' && !story.mediaUrl && (
            <div className="rounded-2xl border border-white/15 bg-black/20 px-6 py-5 text-center text-sm text-white/60">
              {story.type === 'video' ? 'قصة فيديو' : 'قصة صورة'}
              <p className="mt-1 text-xs text-white/35">معاينة الوسائط غير متوفرة</p>
            </div>
          )}
        </div>
        ...
        <div className="absolute bottom-4 left-0 right-0 z-20 px-4">
          <div className="flex items-center gap-2">
            <div className="flex-1 flex items-center gap-2 bg-white/10 rounded-full px-4 py-2.5 border border-white/20">
              ...
              />
              <button onClick={submitReply} disabled={!reply.trim()} className="text-white/70 hover:text-white disabled:opacity-30 transition-colors cursor-pointer" aria-label="إرسال الرد">
        ...
        {story.type === 'video' && story.mediaUrl && (
          <button onClick={() => setMuted((value) => !value)} className="absolute bottom-20 end-4 z-30 rounded-full bg-black/45 p-2 text-white/80 hover:bg-black/70" aria-label={muted ? 'إلغاء كتم الفيديو' : 'كتم الفيديو'}>
            {muted ? '🔇' : '🔊'}
          </button>
        )}
        ...
        {showViewers && (
          <div className="absolute inset-x-4 top-20 z-40 rounded-2xl border border-white/15 bg-black/85 p-4 text-white shadow-xl backdrop-blur-md">
            <div className="flex items-center justify-between">
              <div><p className="font-semibold">مشاهدو القصة</p><p className="text-xs text-white/45">{story.viewedBy.length} مشاهد</p></div>
              <button onClick={() => setShowViewers(false)} className="rounded-full p-1 text-white/60 hover:bg-white/10">✕</button>
            </div>
            <div className="mt-4 space-y-2 text-sm text-white/75">
              {story.viewedBy.length > 0 ? story.viewedBy.map((viewerId) => <div key={viewerId} className="rounded-lg bg-white/5 px-3 py-2">{viewerId === currentUserId ? 'أنت' : `مشاهد ${viewerId}`}</div>) : <p className="text-white/45">لا يوجد مشاهدون بعد.</p>}
            </div>
          </div>
        )}
