...
export function StoryCreator({ userName, userAvatar, onPublish, onClose }: StoryCreatorProps) {
  ...
  const handlePublish = () => {
    ...
    onPublish(content.trim() || `${type === 'image' ? 'قصة صورة' : 'قصة فيديو'}`, bgColor, type, mediaUrl.trim(), privacy)
    onClose()
  }
      ...
      >
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8">
          ...
          <div className="mt-5 flex rounded-full border border-white/15 bg-black/20 p-1 text-xs text-white/60">
            {(['text', 'image', 'video'] as const).map((option) => (
              <button key={option} type="button" onClick={() => setType(option)} className={`flex-1 rounded-full px-3 py-1.5 capitalize transition-colors ${type === option ? 'bg-white text-black' : 'hover:text-white'}`}>
                {option === 'text' ? 'نص' : option === 'image' ? 'صورة' : 'فيديو'}
              </button>
            ))}
          </div>
          {type !== 'text' && (
            <input value={mediaUrl} onChange={(e) => setMediaUrl(e.target.value)} placeholder={`${type === 'image' ? 'رابط الصورة' : 'رابط الفيديو'} (اختياري)`} className="mt-4 w-full rounded-xl border border-white/15 bg-black/20 px-4 py-2 text-sm text-white placeholder:text-white/35 focus:outline-none" />
          )}
          {type === 'image' && mediaUrl && <img src={mediaUrl} alt="معاينة القصة" className="mt-6 max-h-44 max-w-full rounded-xl object-contain" />}
          ...
          <textarea
            ...
            placeholder={type === 'text' ? 'بماذا تفكر؟' : 'أضف تعليقاً (اختياري)'}
          ...
          <label className="mt-5 flex items-center gap-2 text-xs text-white/65">
            ...
            <select value={privacy} onChange={(e) => setPrivacy(e.target.value as typeof privacy)} className="rounded-lg border border-white/15 bg-black/30 px-2 py-1 text-white focus:outline-none">
              ...
              <option value="contacts-except">جهات الاتصال باستثناء…</option>
              <option value="only-share-with">المشاركة مع… فقط</option>
            </select>
          </label>
        </div>
