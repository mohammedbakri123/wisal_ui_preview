...
export function CommunitiesProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo(() => ({
    ...
    addCommunity: (name: string, description: string, visibility: string) => setCommunities((current) => [{ id: `community-${Date.now()}`, name, description: description || 'A new Wisal community.', category: 'New', members: '1', groups: 0, channels: 0, joined: true, owner: true, verified: false, unreadCount: 0, lastMessage: 'Community created.', lastMessageTime: 'Now', visibility }, ...current]),
  }), [communities])
