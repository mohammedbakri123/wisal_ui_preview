...
import { useTheme, type AccentColor, type BubbleStyle, type ChatBackground, type ChatFontSize, type Theme } from '@/app/providers/ThemeProvider'
...
export default function AppearanceSettingsPage() {
  const { accentColor, setAccentColor, bubbleStyle, chatBackground, chatFontSize, theme, setTheme, setBubbleStyle, setChatBackground, setChatFontSize } = useTheme()

  const patterns = [
    { id: 'classic', label: 'شبكة كلاسيكية' },
    { id: 'dots', label: 'نقاط' },
    { id: 'solid', label: 'سادة' },
  ]
      ...
      <PageContainer className="max-w-md mx-auto w-full max-w-full space-y-5 pt-3 sm:pt-4 min-w-0 box-border overflow-x-hidden">
        ...
        <section className="bg-surface rounded-2xl border border-border-light/50 overflow-hidden">
          <div className="p-4 border-b border-border-light/30">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">مظهر التطبيق</h3>
          </div>
          <OptionGrid options={[
            { value: 'dark', label: 'داكن', copy: 'مريح للاستخدام ليلاً. ' },
            { value: 'light', label: 'فاتح', copy: 'واجهة مضيئة وأنيقة.' },
          ]} value={theme} onChange={(value) => setTheme(value as Theme)} />
        </section>

        <section className="bg-surface rounded-2xl border border-border-light/50 overflow-hidden">
          <div className="p-4 border-b border-border-light/30">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">شكل الفقاعة</h3>
          </div>
          <OptionGrid options={[
            { value: 'rounded', label: 'مستديرة', copy: 'زوايا ناعمة للدردشة اليومية.' },
            { value: 'sharp', label: 'حادة', copy: 'كتل محادثة واضحة ومربعة.' },
            { value: 'compact', label: 'مدمجة', copy: 'فقاعات مضغوطة للمحادثات الكثيفة.' },
          ]} value={bubbleStyle} onChange={(value) => setBubbleStyle(value as BubbleStyle)} />
        </section>

        <section className="bg-surface rounded-2xl border border-border-light/50 overflow-hidden">
          <div className="p-4 border-b border-border-light/30">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">لون التمييز</h3>
          </div>
          <div className="p-4">
        ...
        <section className="bg-surface rounded-2xl border border-border-light/50 overflow-hidden">
          <div className="p-4 border-b border-border-light/30">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">خلفية الدردشة</h3>
          </div>
          <div className="p-3 space-y-2">
            {patterns.map((pat) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-3 rounded-xl border transition-colors cursor-pointer text-start ${
        ...
        <section className="bg-surface rounded-2xl border border-border-light/50 overflow-hidden">
          <div className="p-4 border-b border-border-light/30">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">حجم نص الرسائل</h3>
          </div>
          <OptionGrid options={[
            { value: 'small', label: 'صغير', copy: 'عرض المزيد من الرسائل.' },
            { value: 'standard', label: 'عادي', copy: 'قراءة متوازنة يومية.' },
            { value: 'large', label: 'كبير', copy: 'قراءة أكثر راحة.' },
