        ...
        <section className="pt-6 sm:pt-8 pb-1">
          <div className="animate-reveal">
            ...
            <p className="text-sm text-muted-foreground/50 mt-2 max-w-md leading-relaxed">
              Find conversations, messages, and contacts across Wisal.
            </p>
          </div>
        ...
        <div className="pt-5 sm:pt-6 space-y-4">
          ...
          <div className="group flex items-center bg-surface/60 border border-border-light/20 rounded-2xl px-4 py-3 focus-within:border-accent/30 focus-within:ring-2 focus-within:ring-accent/10 transition-all duration-300">
            ...
            {query && (
              <button 
                onClick={() => setQuery('')}
                className="p-0.5 rounded-full bg-muted/50 hover:bg-muted text-muted-foreground/60 hover:text-foreground transition-colors cursor-pointer ml-2 shrink-0"
              >
          ...
          {query === '' ? (
            <div className="space-y-4">
              <div className="flex items-center gap-2 px-1">
                ...
                <span className="text-[11px] sm:text-xs font-semibold text-muted-foreground/50 uppercase tracking-[0.15em]">Recent Searches</span>
              </div>
              <div className="space-y-1.5">
                {recentSearches.map((term, i) => (
                  <button
                    ...
                    className="group flex w-full items-center justify-between rounded-2xl border border-border-light/10 bg-surface/30 p-3.5 sm:p-4 text-left text-sm transition-all duration-200 hover:bg-surface/60 hover:border-border-light/30 cursor-pointer"
                  >
                    <span className="text-foreground/70 group-hover:text-foreground font-medium transition-colors">{term}</span>
                    <svg className="h-4 w-4 text-muted-foreground/20 group-hover:text-muted-foreground/50 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between px-1">
                ...
                {filteredChats.length > 0 && (
                  <span className="text-[10px] text-muted-foreground/20">Select a chat to open</span>
                )}
              </div>
              {filteredChats.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-xs text-muted-foreground/40 mt-1 max-w-xs">Try a different search term or check your spelling.</p>
                </div>
              ) : (
                <div className="overflow-hidden rounded-2xl border border-border-light/10 bg-surface/20 divide-y divide-border-light/5">
                  {filteredChats.map((chat) => (
                    <button
                      ...
                      className="group w-full flex items-center gap-3.5 p-3.5 sm:p-4 hover:bg-surface/40 transition-all text-left cursor-pointer"
                    >
