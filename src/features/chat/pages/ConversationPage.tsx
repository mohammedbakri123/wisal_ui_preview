...
export default function ConversationPage() {
  ...
  const [replyingTo, setReplyingTo] = useState<
    import("@/core/types").Message | null
  >(null);
  ...
  const { messages, isLoading, isReplying, error, clearMessages } =
  ...
  const highlightMessageId = (
    location.state as { highlightMessageId?: string } | null
  )?.highlightMessageId;
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-black text-[#e7e9ea]">
      ...
      <main className="relative flex-1 overflow-y-auto scrollbar-thin bg-black">
        {isLoading ? (
          ...
          <MessageList
            ...
            onOpenMember={
              conversation.type !== "direct" ? setSelectedMember : undefined
            }
          />
        )}
      </main>
      ...
      <footer className="shrink-0 bg-black">
        {error && (
          <p className="px-4 py-1 text-center text-xs text-[#f4212e]">
            {error}
          </p>
        )}
        <MessageInput
        ...
        {selectedMember && (
          <div className="text-center">
            <Avatar
              ...
              size="xl"
              online={selectedMember.isOnline}
            />
            <h2 className="mt-3 text-xl font-bold">{selectedMember.name}</h2>
            <p className="mt-1 text-sm text-[#71767b]">عضو في المجموعة</p>
            <p className="mt-4 text-sm leading-relaxed text-[#71767b]">
              {selectedMember.bio ?? "لا توجد نبذة بعد."}
            </p>
            <div className="mt-5 flex gap-2">
              <Button
                ...
                onClick={() =>
                  navigate(
                    `/profile/${selectedMember.name.toLowerCase().replace(/\s+/g, "")}`,
                  )
                }
              >
                عرض الملف الشخصي
