      ...
      <PageContainer className="w-full px-4 pt-3 pb-8">
        <div className="mx-auto max-w-2xl">
          ...
          <header className="mb-6 mt-2">
            ...
            <h1 className="mt-1 text-2xl font-bold">إدارة المشرفين</h1>
            <p className="mt-2 text-sm leading-relaxed text-[#71767b]">اختر من يمكنه نشر التحديثات والإشراف على المنشورات وإدارة هذه القناة.</p>
          </header>

          <section className="overflow-hidden rounded-2xl border border-[#2f3336] bg-[#16181c]">
            <div className="border-b border-[#2f3336] p-4">
              <h2 className="text-[15px] font-bold">المشرفون الحاليون</h2>
              <p className="mt-1 text-xs text-[#71767b]">لا يمكن إزالة المالك حتى يتم نقل الملكية.</p>
            </div>
            {admins.map((user) => (
              <div key={user.id} className="flex items-center gap-3 border-b border-[#2f3336] p-4 last:border-b-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-[#71767b]">{user.id === ownerId ? 'المالك' : 'مشرف'}</p>
                </div>
                {user.id !== ownerId && <Button size="sm" variant="danger" onClick={() => removeAdmin(user.id)}>إزالة</Button>}
                {user.id === ownerId && <span className="text-xs font-bold text-[#1d9bf0]">المالك</span>}
              </div>
            ))}
          </section>

          <section className="mt-6 rounded-2xl border border-[#2f3336] bg-[#16181c] p-4">
            <h2 className="text-[15px] font-bold">إضافة مشرف</h2>
            <div className="mt-3 flex flex-col gap-2 sm:flex-row">
              ...
              <Button size="sm" onClick={addAdmin} disabled={!selectedUserId}>إضافة مشرف</Button>
            </div>
          </section>

          <section className="mt-6 rounded-2xl border border-[#2f3336] bg-[#16181c] p-4">
            <h2 className="text-[15px] font-bold">نقل الملكية</h2>
            <p className="mt-1 text-xs leading-relaxed text-[#71767b]">يمنح نقل الملكية للمشرف المحدد التحكم الكامل في هذه القناة.</p>
            <div className="mt-3 flex flex-col gap-2 sm:flex-row">
              ...
              <span className="inline-flex h-10 items-center justify-center rounded-full border border-[#2f3336] px-4 text-xs text-[#71767b]">تم الحفظ محليًا</span>
            </div>
          </section>
        </div>
      </PageContainer>
